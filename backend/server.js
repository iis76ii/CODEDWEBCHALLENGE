...
const FRONTEND_URL = process.env.FRONTEND_URL || 'https://codedwebsite.vercel.app';
const DB_PATH      = path.join(__dirname, 'db.json');

// ── Crash guards ──
process.on('uncaughtException',  e => { console.error('CRASH:', e.message); process.exit(1); });
...
(function seedDemo() {
  ...
  ].forEach(([name,platform,total_hrs,progress,status,emoji,color]) => {
    const db3 = readDB();
    db3.courses.push({ id:nextId(), user_id:uid, name, platform, total_hrs, progress, status, emoji, color });
    writeDB(db3);
  });
  [
  ...
  ].forEach(([name,current,target,deadline,emoji,color]) => {
    const db3 = readDB();
    db3.goals.push({ id:nextId(), user_id:uid, name, current, target, deadline, emoji, color });
    writeDB(db3);
  });
  [2.5,1.8,0.5,1.2,0,0.5,0].forEach((hours,i) => {
    const d = new Date(); d.setDate(d.getDate()-(6-i));
    const db3 = readDB();
    db3.activity.push({ id:nextId(), user_id:uid, hours, logged_date:d.toISOString().split('T')[0] });
    writeDB(db3);
  });
...
app.post('/api/activity/log', auth, (req, res) => {
  ...
  if (ex) ex.hours += hours; else db.activity.push({ id:nextId(), user_id:req.user.id, hours, logged_date:today });
  writeDB(db);
  res.json({ success:true });
});
...
app.post('/api/ai/chat', auth, async (req, res) => {
  ...
  if (!OPENROUTER) return res.status(503).json({ error:'AI not configured — add OPENROUTER_API_KEY in Render environment variables' });
  const db = readDB();
